...
From the Bar Chart \ref{fig:f-measure}, we see that the C\_LOF algorithm performs only better in the Recall part  but left much to improve in Precision part as well as the overall F-Score. This implies that the C\_LOF generate a large number of false positive results. I think this may be highly related to the choosing of the hyper-parameter $\theta$, which decides the sensitivity of deciding being outlierness. A small number of $\theta$ would lead to new data point being classified as outlier even if its LOF score deviate little bit from the rest. Ideally, this hyper-parameter should be adjusted dynamically as more data being observed. 

%A more suitable solution to this problem would be when deciding the outlier, we not only should consider the historical data along with the data from current window, but also consider the future data! This means the algorithm should not report outliers immediately after calculating its LOF scores, but rather treat it as a candidate outlier and only report it as an outlier after observing future data points of window size $n$ if it still satisfy the outlier conditions. 
...
Figure \ref{fig:scalability-D} presents the average time (scaled between 0 and 1) spent on C\_LOF and LOF algorithms respectively to process a random generated dataset with respect to increasing number of attributes (up until 64) over the dataset. We fix the settings on the number of data size N to 1024 and the number of K in K nearest neighbors to 20 in order to make comparisions. 
