...
Figure \ref{fig:scalability-D} presents the average time (scaled between 0 and 1) spent on C\_LOF and LOF algorithms respectively to process a random generated dataset with respect to increasing number of attributes (up until 64) over the dataset. We fix the settings on the number of data size N to 1024 and the number of K in K nearest neighbors to 20 before making comparisons. From the graph, we hardly notice any linear increase on the time spent on processing of the data with respect to the increasing of dimension in dataset in C\_LOF algorithm. On the other hand, the sequential implementation of LOF seems to scales well so far with respect to data dimension but the overall time spent on processing of data is significantly longer when data dimension increases compared to C\_LOF implementation. 
...
Due to the limitation on the hardware memory resource in this project, I did not process data with dimension higher than 64, and data size higher than 1024. It will be interesting to see how the runtimes of both implementations will behave on a larger dimension of data and larger data size. 
% ############################################################################
